...
class MetadataConfig 
{
    ...
    public function warmup(array $value = []): \Symfony\Config\JmsSerializer\InstancesConfig\Metadata\WarmupConfig
    {
        if (null === $this->warmup) {
            $this->_usedProperties['warmup'] = true;
            $this->warmup = new \Symfony\Config\JmsSerializer\InstancesConfig\Metadata\WarmupConfig($value);
    ...
    public function fileCache(array $value = []): \Symfony\Config\JmsSerializer\InstancesConfig\Metadata\FileCacheConfig
    {
        if (null === $this->fileCache) {
            $this->_usedProperties['fileCache'] = true;
            $this->fileCache = new \Symfony\Config\JmsSerializer\InstancesConfig\Metadata\FileCacheConfig($value);
        } elseif (0 < \func_num_args()) {
    ...
    public function directory(string $name, array $value = []): \Symfony\Config\JmsSerializer\InstancesConfig\Metadata\DirectoryConfig
    {
        if (!isset($this->directories[$name])) {
            $this->_usedProperties['directories'] = true;
            $this->directories[$name] = new \Symfony\Config\JmsSerializer\InstancesConfig\Metadata\DirectoryConfig($value);
    ...
    public function __construct(array $value = [])
    {
        if (array_key_exists('warmup', $value)) {
            $this->_usedProperties['warmup'] = true;
            $this->warmup = new \Symfony\Config\JmsSerializer\InstancesConfig\Metadata\WarmupConfig($value['warmup']);
            unset($value['warmup']);
        }
        ...
        if (array_key_exists('file_cache', $value)) {
            $this->_usedProperties['fileCache'] = true;
            $this->fileCache = new \Symfony\Config\JmsSerializer\InstancesConfig\Metadata\FileCacheConfig($value['file_cache']);
        ...
        if (array_key_exists('directories', $value)) {
            $this->_usedProperties['directories'] = true;
            $this->directories = array_map(function ($v) { return new \Symfony\Config\JmsSerializer\InstancesConfig\Metadata\DirectoryConfig($v); }, $value['directories']);
